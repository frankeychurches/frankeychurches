...
static float voltage_bus, current_mA, load_power  = 0;   
static String volt_str, curr_str, pow_str;    
...
static volatile bool activateMotors;
...
<head>
  ...
  <style>
    ...
    button { position:absolute; left:100px; top:475px;}
...
void INA_Measure(void* parameters)
{
  while(1)
  {
    ...
    load_power = ina219.getPower_mW();
    //String variables for HTML interface communication  
    volt_str = String(voltage_bus); curr_str = String(current_mA); pow_str = String(load_power);
    #ifdef DEBUG
...
String processor(const String& var){
  if(var == "TEMPERATURE"){ return temp_str;}
  ...
  else if(var == "INA_VOLT"){ return volt_str;}
  else if(var == "INA_CURR"){ return curr_str;}
  else if(var == "INA_POW"){ return pow_str;}
...
String processor_button(const String& var){
  //Serial.println(var);
  if(var == "BUTTONPLACEHOLDER")
  {
    String buttons ="";
    String outputStateValue = outputState();
    buttons+= "<h4>Motor Button<span id=\"outputState\"></span></h4><label class=\"switch\"><input type=\"checkbox\" onchange=\"toggleCheckbox(this)\" id=\"output\" " + outputStateValue + "><span class=\"slider\"></span></label>";
    return buttons;
  }
  return String();
}
...
void WiFi_HTML_setup()
{
  ...
  server.on("/ina_volt", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", volt_str.c_str());
  });
  server.on("/ina_curr", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", curr_str.c_str());
  });
  server.on("/ina_pow", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", pow_str.c_str());
  });
