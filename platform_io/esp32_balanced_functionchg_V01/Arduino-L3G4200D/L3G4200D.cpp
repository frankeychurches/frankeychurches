...
void L3G4200D::calibrate(uint8_t samples)
{
	...
	Serial.print("gx= ");
    Serial.print(r.XAxis);
    Serial.print("gy= ");
    Serial.print(r.YAxis);
    Serial.print("gz= ");
    Serial.println(r.ZAxis);
	delay(5);
    }
...
void L3G4200D::setThreshold(uint8_t multiple)
{
    ...
    Serial.print("tmulx= ");
    Serial.print(t.XAxis);
    Serial.print("tmuly= ");
    Serial.print(t.YAxis);
    Serial.print("tmulz= ");
    Serial.println(t.ZAxis);
    } else
    {
	// No threshold
	t.XAxis = 0;
...
/*  FCOLLAVE 05/2021--VALORES SELECCIONABLES PARA EL DPS DEL GIROSCOPIO    
    switch(scale)
    {
	case L3G4200D_SCALE_250DPS:       DPS == DEGREES PER SECOND
	    dpsPerDigit = .00875f;
	    break;
	case L3G4200D_SCALE_500DPS:
	    dpsPerDigit = .0175f;
